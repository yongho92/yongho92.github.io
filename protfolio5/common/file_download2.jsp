...
%><%@ include file="/inc/init.jsp"%><%
    ...
    sql.setLength(0);
    sql.append("select * from product where seq="+seq);
    parameters.set("query", sql.toString());
    parameters.set("params", null);
    singleDE01 = exec.getGeneralSingle(parameters);

    String nextPage = "";
    if(singleDE01.isEmpty("server_imagename") && singleDE01.isEmpty("server_expand_imagename"))
    {
    ...
    {
        String path = bizProp.getServerPath("etc");
        path = path + "/" + singleDE01.print("server_expand_imagename");
        if(!new File(path).canRead()){
            errorMsg("요청하신 첨부파일이 서버에 존재하지 않습니다.");
            return;
        }
        ...
        parameters.set("server_path", path);
        parameters.set("cf", singleDE01.print("client_expand_imagename"));
