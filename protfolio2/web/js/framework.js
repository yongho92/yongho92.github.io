...
function getDatepickerOption(type){
    var ret = {
		dateFormat : "yy-mm-dd",
		showOn: 'both',
		buttonImage: documentRoot + 'biznet_2018/web/images/admin/btn_calendar.gif',
...
function _formInit(){
    for(var z=0 ; z<document.forms.length ; z++){
        ...
        for(var i=0 ; i<form.elements.length ; i++){
            ...
            if(obj.getAttribute("required") != null && nvl(obj.title, "") != "") obj.title = "�꾩닔�낅젰�ы빆�낅땲��.";
			...
			if(obj.type.toUpperCase() == "TEXT" || obj.type.toUpperCase() == "PASSWORD"){
				...
				if(minLength != null){
					if($.trim(value).length < minLength){
						alert("理쒖냼 " + minLength + "湲��� �댁긽 �낅젰�섏뀛�� �⑸땲��.");
						obj.focus();
						//obj.style.backgroundColor = highlight_color;
						return false;									
					}
...
function _onfocusout()
{
	...
	if(obj.getAttribute("MINLENGTH") != null)
	{
		...
		{
			alert("理쒖냼�� " + minLength + "湲��먯씠�� �낅젰�섏뀛�� �⑸땲��.");
