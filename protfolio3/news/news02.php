<?
    ...
    $parameters->set("sc_board_seq", nvl($parameters->print2("sc_board_seq"),"2"));
...
function submitPage(page){
    $("#act").val("");
    $("#page").val(page);
    frm.submitForm3(window, "news02.php");
}
	...
	<div class="contents">
		...
		<div class="exp_ser_m">
			<div class="board_ser10">
				<span class="board_all10" id="boa_set10_text">알람게시판</span>
				<label for="boa_set10" class="sor_hide10">알람게시판</label>
...
<?
    for($i=0; $i<$list01["selectCnt"]; $i++)
    {
        ...
        {
            $img = "../web/images/common/news_default_img.jpg";
        }
?>
						<li>
							<div class="bor_txt">
								<span><?=substr($row["regi_date"], 0, 10)?></span>
								<h4><a href="javascript:goView('<?=$row["SEQ"]?>');"><?=$row["title"]?></a></h4>
							</div>
							<div class="bor_img">
								<a href="javascript:goView('<?=$row["SEQ"]?>');"><img src="<?=$img?>" width="252" alt=""/></a>
							</div>
						</li>
<?
    }
